...
export default function SuccessView(props) {
  const { listItems, handleAnyCheckboxClick } = props;
  const [selectAll, setSelectAll] = useState(true);
  const [secFields, setCheckedItems] = useState(props.listItems);
  const [selectedItems, setSelectedItems] = useState(0);
  // console.log(secFields ,'secFields s')

  // console.log('listItems S',listItems)

  useEffect(() => {
    setCheckedItems(props.listItems);
  }, [props.listItems]);
  useEffect(() => {
    let count = props.listItems.filter((item) => item.checked).length;
  ...
  const handleSelectAllItems = () => {
    if (selectAll) {
      let filteredList = secFields.map((item) => ({ ...item, checked: true }));
      ...
      let filteredList = secFields.map((item) => ({ ...item, checked: false }));
      let isChecked = filteredList.some((each) => each.checked === true);
      handleAnyCheckboxClick(isChecked, filteredList);
      setCheckedItems(filteredList);
      setSelectedItems(0);
  ...
  const handleCheckboxChange = (id) => {
    const updatedItems = secFields.map((item) => {
      if (item.id === id) {
        return {
          ...item,
      ...
      <div className="selected-card">
        <div className="count-container">
          <CheckBox
            ...
            id="selectAll"
          />
          <label className="items-selected" htmlFor="selectAll">
            Select All
          </label>
        </div>
      ...
      <ul className={classes.listDataContainer}>
        {secFields.map((each) => (
          <li
            className="list-item"
            key={each.id}
            style={each.checked ? { backgroundColor: "#242C40" } : null}
          >
            <CheckBox
              id={each.id}
              checked={each.checked}
              sx={{ color: "#9497A1", width: "16px", height: "16px" }}
              classes={{ root: classes.checkbox }}
              onChange={() => handleCheckboxChange(each.id)}
            />
            <label className="list-content" htmlFor={each.id}>
              {each.fieldName}
            </label>
          </li>
        ))}
